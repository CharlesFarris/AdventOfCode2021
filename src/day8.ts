...
function dayEightPartOne(): void {
    ...
    for (const input of values) {
        ...
        const outputs: string[] = tokens[1]
            ...
            .split(" ")
            .map(t => t.trim());
        count = outputs.reduce((localCount, output) => {
...
function containsPattern(left: string, right: string): boolean {
    return (
        right.split("").reduce((count, character) => {
            return count + (left.indexOf(character) === -1 ? 0 : 1);
        }, 0) === right.length
    );
}
...
function dayEightPartTwo(): void {
    ...
    for (const input of values) {
        ...
        patterns.splice(patterns.indexOf(mappedPatterns[4]), 1);
        ...
        sum += parseInt(
            outputs.reduce((buffer: string, output: string) => {
                return buffer + mappedPatterns.indexOf(output).toString();
            }, "")
        );
    }
    console.log("Sum: " + sum);
