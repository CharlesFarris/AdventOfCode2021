...
class Map3d {
    constructor(
        ...
        this.rangeX = new Range(startX, endX);
        this.sizeX = this.rangeX.end - this.rangeX.start + 1;
        this.rangeY = new Range(startY, endY);
        this.sizeY = this.rangeY.end - this.rangeY.start + 1;
        this.rangeZ = new Range(startZ, endZ);
        this.sizeZ = this.rangeZ.end - this.rangeZ.start + 1;

        this.values = new Array(this.sizeX * this.sizeY * this.sizeZ).fill(initialValue);
    }

    isInside(x: number, y: number, z: number): boolean {
        return this.rangeX.contains(x) && this.rangeY.contains(y) && this.rangeZ.contains(z);
    }

    getIndex(x: number, y: number, z: number): number | undefined {
        return this.isInside(x, y, z)
            ? x -
                  this.rangeX.start +
                  (y - this.rangeY.start) * this.sizeX +
                  (z - this.rangeZ.start) * this.sizeX * this.sizeY
            : undefined;
    }

    getValue(x: number, y: number, z: number): number | undefined {
    ...
    readonly rangeX: Range;
    readonly rangeY: Range;
    readonly rangeZ: Range;
...
function executeRebootStep(region: Map3d, line: string): void {
    let value = -Infinity;
    ...
    if (resultX.status) {
        ...
        if (resultY.status) {
            const resultZ = intersection(region.rangeZ, ranges[2]);
            if (resultZ.status) {
                for (let z = resultZ.range.start; z <= resultZ.range.end; z++) {
                    for (let y = resultY.range.start; y <= resultY.range.end; y++) {
                        for (let x = resultX.range.start; x <= resultX.range.end; x++) {
                            region.setValue(x, y, z, value);
                        }
                    }
                }
            }
...
function dayTwentyTwoPartOne(): void {
    const region = new Map3d(-50, -50, -50, 50, 50, 50, 0);
    for (const step of steps) {
        executeRebootStep(region, step);
    }

    const count = region.values.reduce((localCount: number, value: number) => {
...
function dayTwentyTwoPartTwo(): void {
    ...
    for (const step of steps) {
        ...
        if (step.startsWith("on")) {
            ...
            const offCuboid = new Cuboid(ranges[0], ranges[1], ranges[2]);
            ...
            while (stack.length > 0) {
                ...
                const subtracted = current.subtract(offCuboid);
