...
const instructions: string[] = ["fold along y=7", "fold along x=5"];
*/
...
function dayThirteenPartOne(): void {
    const points: Point[] = dots.map((line: string) => {
        ...
        if (instruction.startsWith("fold along y=")) {
            ...
            for (let x = 0; x < foldMap.width; x++) {
                for (let y = 0; y < foldMap.height; y++) {
                    const value = foldMap.getValue(x, y);
                    if (value === undefined) {
                        throw new Error("value undefined");
                    }
                    if (value !== 0) {
                        newMap.setValue(x, y + startY, value);
                    }
                }
            }
            paper = newMap;
            ...
            for (let y = 0; y < foldMap.height; y++) {
                for (let x = 0; x < foldMap.width; x++) {
                    ...
                    if (value !== 0) {
                        newMap.setValue(x + startX, y, value);
                    }
                }
            }
            paper = newMap;
        }
        const count: number = paper.values.reduce((localCount: number, value: number) => {
